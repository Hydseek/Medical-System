...
final class ResetPasswordController extends AbstractController
{
    ...
    private function processSendingPasswordResetEmail(Request $request, $user): Response
    {
        try {
            ...
            $this->addFlash('reset_password_error', sprintf(
                'An error occurred while requesting password reset - %s',
                $e->getReason()
            ));
    ...
    {
        // Display a message asking the user to check their mailbox
    ...
    public function reset(
        ...
        if (null === $user) {
            $this->addFlash('reset_password_error', 'Invalid or expired token.');
            return $this->redirectToRoute('app_forgot_password_request');
        }
